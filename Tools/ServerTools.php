...
class ServerTools
{
    ...
    {
        // Based on http://php.net/manual/en/security.magicquotes.disabling.php, example 2
        ...
        while (list($key, $val) = each($list)) {
            foreach ($val as $k => $v) {
                unset($list[$key][$k]);
                if (is_array($v)) {
                    $list[$key][stripslashes($k)] = $v;
                    $list[] = & $list[$key][stripslashes($k)];

                } else {
                    $list[$key][stripslashes($k)] = stripslashes($v);
                }
            }
        }
