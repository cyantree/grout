...
class PdoConnection extends DatabaseConnection {
	...
	public function __construct() {
		...
		if(self::$_filterArraySearch === null) self::$_filterArraySearch = array('\\', '"', "\0", "\n", "\r", chr(26), "'");
   }

   public function connectMySql($host, $user, $pass, $database, $charset = 'utf8', $port = 3306){
		if($this->_connection) return true;

		$this->_connection = new PDO('mysql:host='.$host.';port='.$port.';dbname='.$database, $user, $pass, array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES '.$charset));
	...
	public function query($query, $args = null, $flags = 0){
		if(!($flags & (Database::FILTER_ALL|Database::FILTER_COLUMN|Database::FILTER_FIELD|Database::FILTER_ROW))) $flags = $flags|Database::FILTER_ALL;

		if(!($flags & (Database::TYPE_ASSOC|Database::TYPE_NUM))) $flags = $flags|Database::TYPE_ASSOC;
		...
		if($q === false){
			$e = $this->_connection->errorInfo();
			trigger_error($e[2], E_USER_ERROR);
		}
		$this->_errorHandler->unRegister();

		if(!$q || $q->rowCount() == 0){
			if(($flags & Database::FILTER_COLUMN) || ($flags & Database::FILTER_ARRAY) || ($flags & Database::FILTER_ROW)) return array();

			if($flags & Database::FILTER_FIELD) return null;
		...
		if($flags & Database::FILTER_ARRAY){
			$res = array();
			while($data = $reader->read()) $res[] = $data;
	...
	public function exec($query, $args = null, $flags = 0){
		...
		$this->_errorHandler->register($query);
		$affectedRows = $this->_connection->exec($query);
	...
	public function insert($table, $fields, $values, $nestedRows = false, $rowsPerInsert = 100, $returnQueries = false){
		...
		if($nestedRows){
			...
			foreach($values as $row){
				...
				if($rowCount >= $rowsPerInsert){
					if($returnQueries) $queries[] = $this->prepareQuery($q, $args);
					else $this->exec($q, $args);
		...
		if($returnQueries) return $queries;
	...
	public function prepareQueryFilterCallback($replaces){
		foreach($replaces as $key => $replace){
			...
			if($type == 's')
				...
				$r = array();
				foreach($val as $v) $r[] = '"'.str_replace(self::$_filterArraySearch, self::$_filterArrayReplace, $v).'"';
